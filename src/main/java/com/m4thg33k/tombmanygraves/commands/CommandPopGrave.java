...
public class CommandPopGrave {

	public static void register(CommandDispatcher<CommandSource> dispatcher) {
		LiteralArgumentBuilder<CommandSource> builder = Commands.literal("tmg_popgrave").requires((source) -> {
		...
		builder.then(Commands.argument("pos", BlockPosArgument.blockPos()).executes((context) -> {
			...
			if (tile == null || !(tile instanceof TileGrave)) {
				...
				((TileGrave) tile).dropGraveContentsHere();
			}
			return 0;
		}));
